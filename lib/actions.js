...
module.exports.waitFor = function waitFor(event, data, timeout) {
  this.event = event;
  this.data = data;
  ...
  this.execute = function execute(client, callback) {
    ...
    var eventHandler = function waitForEventHandler(message) {
      if (data == null || _.isEqual(message, data)) {
    ...
    var waitForTimeout = setTimeout(function() {
      ...
      message = message + 'was not received in ' + timeout + ' milliseconds';
...
module.exports.failIfReceived = function failIfReceived(event, data, timeout) {
  this.event = event;
  this.data = data;
  this.timeout = timeout;
...
module.exports.failIfReceivedNonBlocking = function failIfReceivedNonBlocking(event, data, timeout) {
  this.event = event;
  this.data = data;
  this.timeout = timeout;
